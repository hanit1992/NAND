...
class Parser:
    ...
    file_to_parse = None # the input file object to read from
    current_line = '' # the current line we wish to read
    program_commands = [] # a table with all the commands
    ...
    def command_type(self):
        name = self.current_line.split(' ')[0] # there has to be a white space
        if name == 'push':
    ...
    def parse_command(self):
        ...
        # getting the next command object
    ...
    def parse_all_commands(self):
        for line in self.file_to_parse:
            ...
            if line != '' and line[0] != '/' and line != '\n': # skipping on the empty lines
                # first cleaning non empty lines from notes
                temp_line = line.split('//')[0]
                final_line = temp_line.split('\n')[0]
                the_line = final_line.strip()
                self.current_line = the_line # putting in the current line the next line to process (ready to be process)
                self.parse_command()