...
def write_init(output_file_object):
    """
    writes to the file the initial assembly orders. being called first. it's an assembly text that i'm adding,
    not some translation of a vm file. this function will also set the output file object to write on
    ...
    text = '@256' + '\n' + \
           'D=A' + '\n' + \
           '@SP' + '\n' + \
           'M=D' + '\n' + '\n' # setting sp to point to 256
    output_file.write(text)


def write_all_commands(commands_list,output_file_object):
    """
    recieves a list of commands objects and writes the equivelent hack command to the file in the order of the
    list
    :param commands_list: list of commands from the parser
    :param output_file_object
    """
    # global output_file
...
def write_push_for_static_segment(command):
    ...
    i = command.get_arg_2()
    current_file = current_function_name.split('.')[0]
    text_to_write = '@' + current_file + '.' + str(i) + '\n' + \
                    'D=M' + '\n' + \
                    PUSH_FROM_D_VALUE
    output_file.write(text_to_write)
